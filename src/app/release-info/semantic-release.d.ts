...
export type ReleaseInfoSummary = Pick<
  ReleaseInfo,
  'fake' | 'preview' | 'nextRelease'
>
...
type PatchedLastRelease = Omit<LastRelease, 'channels'> & {
  channels: LastRelease['channels'] | null[]
}
...
type PatchedCommitCommitter = Omit<Commit['committer'], 'short'> & {
  date: string
}
type PatchedNextRelease = Omit<NextRelease, 'channel'> & {
  channel: NextRelease['channel'] | null
}
type PatchedRelease = Omit<Release, 'pluginName'> & {
